...
ca_fish_list <- read.table(here("Data","CA_fish_species","CA_fish_list_20201204.csv"), header = 1, sep = ",", stringsAsFactors = F)

## Species Monitored by PISCO, KFM, and RCCA
monitored <- read.table(here("Data","CA_fish_species","Monitored_Species_list_20201204.txt"), header = 1, sep = "\t", stringsAsFactors = F)
...
```{r}
ca_fish_list$Miller_Lea_Combo_03262020 %>% unique() %>% length()
#864 species in CA
```
...
setdiff(FishCARD_df$Species,ca_fish_list$Miller_Lea_Combo_03262020) -> barcodes_with_genus_level_ids
...
```{r}
intersect(ca_fish_list$Miller_Lea_Combo_03262020,GenBank_db_taxonomy_species$Species) -> ca_species_already_barcoded

ca_species_already_barcoded %>%  length
```
#### CA Fish Species Missing MiFish *12S* References Prior to Barcoding Efforts
```{r}
setdiff(ca_fish_list$Miller_Lea_Combo_03262020,GenBank_db_taxonomy_species$Species) -> ca_species_missing_barcodes_pre_fishcard
...
setdiff(ca_fish_list$Miller_Lea_Combo_03262020,local_db_species_list) -> Ca_fish_missing_mifish_barcodes
...
```{r}
length(fishcard_only_species$Species)/length(ca_fish_list$Miller_Lea_Combo_03262020)*100
```

#### Percent of CA Fish species Now Barcoded

```{r}
length(local_db_species_list)/length(ca_fish_list$Miller_Lea_Combo_03262020)*100
...
setdiff(local_asv$Species, ca_fish_list$Miller_Lea_Combo_03262020)
#all species Native
```
...
setdiff(fish_species_merged_local$Species, ca_fish_list$Miller_Lea_Combo_03262020)
```
#### Forward
```{r}
#forward Total Reads
...
setdiff(fish_species_forward_local$Species, ca_fish_list$Miller_Lea_Combo_03262020)
```
...
setdiff(fish_species_reverse_local$Species, ca_fish_list$Miller_Lea_Combo_03262020)
```
...
#California Native Assigned Species
intersect(genbank_asv$Species, ca_fish_list$Miller_Lea_Combo_03262020)
#species Native
```
### Supplemental Results
...
intersect(fish_species_merged_genbank$Species, ca_fish_list$Miller_Lea_Combo_03262020)
```
#### Forward
```{r}
#forward Total Reads
...
intersect(fish_species_forward_genbank$Species, ca_fish_list$Miller_Lea_Combo_03262020)
```
...
intersect(fish_species_reverse_genbank$Species, ca_fish_list$Miller_Lea_Combo_03262020)
```
...
#California Native Assigned Species
intersect(global_asv$Species, ca_fish_list$Miller_Lea_Combo_03262020)
#species Native
```
### Supplemental Results
...
intersect(fish_species_merged_global$Species, ca_fish_list$Miller_Lea_Combo_03262020)
```
#### Forward
```{r}
#forward Total Reads
...
intersect(fish_species_forward_global$Species, ca_fish_list$Miller_Lea_Combo_03262020)
```
...
intersect(fish_species_reverse_global$Species, ca_fish_list$Miller_Lea_Combo_03262020)
```
...
```
### Explore Global Sequences without Species Level ID
```{r}
setdiff(global_no_species$c19_fishcard_seq_number, comparison_2_delete$c19_fishcard_seq_number) -> c19_diff
comparison_2 %>% 
  filter(., c19_fishcard_seq_number %in% c19_diff)
```
### Explore GenBank Sequences without Species Level ID
```{r}
setdiff(genbank_no_species$Oct2019_seq_number, comparison_2_delete$Oct2019_seq_number) -> Oct2019_diff
comparison_2 %>% 
  filter(., Oct2019_seq_number %in% Oct2019_diff)
```

### Explore Local Sequences without Species Level ID

```{r}
setdiff(local_no_species$fishcard_12S_all_seq_number, comparison_2_delete$fishcard_12S_all_seq_number) -> fish_diff
comparison_2 %>% 
  filter(., fishcard_12S_all_seq_number %in% fish_diff)
```
...
setdiff(genbank_only_v_local_diff, ca_fish_list$Miller_Lea_Combo_03262020)
# 13 species are not CA natives that were assigned to CA species after including additional barcodes

intersect(genbank_only_v_local_diff, ca_fish_list$Miller_Lea_Combo_03262020)
...
setdiff(genbank_only_v_global_diff, ca_fish_list$Miller_Lea_Combo_03262020)
# 12 species are not CA natives that were assigned to CA species after including additional barcodes

intersect(genbank_only_v_global_diff, ca_fish_list$Miller_Lea_Combo_03262020)
# 4 species are CA native taxa that changed after including additional barcodes
```
...
setdiff(genbank_species_unique, ca_fish_list$Miller_Lea_Combo_03262020)
...
```{r}
intersect(genbank_species_unique, ca_fish_list$Miller_Lea_Combo_03262020)
```
...
```{R}
ca_fish_list %>% 
  mutate(., Local_DB = if_else(Miller_Lea_Combo_03262020 %in% FishCARD_db$Species,1, 0)) %>% 
  mutate(., GenBank_DB = if_else(Miller_Lea_Combo_03262020 %in% GenBank_db$Species,1, 0)) %>% 
  mutate(., Global_DB = if_else(Miller_Lea_Combo_03262020 %in% Global_db$Species,1, 0)) %>% 
  arrange(Miller_Lea_Combo_03262020) %>% 
  mutate(., species_added_by_our_efforts = if_else(Miller_Lea_Combo_03262020 %in% species_added_by_our_efforts,1, 0)) %>% 
  left_join(Global_db, by=c("Miller_Lea_Combo_03262020"="Species")) %>% 
  dplyr::select(Class,Miller_Lea_Combo_03262020, Local_DB,GenBank_DB,Global_DB,species_added_by_our_efforts)-> supplemental_table_1
